...
strata = 1
result = StratifiedMonteCarlo(interval[0], interval[1], N, strata) # Crude Monte Carlo, only 1 stratum
print('\nCrude Monte Carlo ({:d} samples)'.format(N))
print('momentInertia = {:.2e} +- {:4.2e}'.format(result['integral'], result['error']))

strata = 4
result = StratifiedMonteCarlo(interval[0], interval[1], N, strata) # Stratified Monte Carlo wtih 4 strata
print('\nStratified Monte Carlo ({:d} strata)'.format(strata))
print('momentInertia = {:.2e} +- {:4.2e}'.format(result['integral'], result['error']))

strata = 16
result = StratifiedMonteCarlo(interval[0], interval[1], N, strata) # Stratified Monte Carlo with 16 strata
print('\nStratified Monte Carlo ({:d} strata)'.format(strata))
print('momentInertia = {:.2e} +- {:4.2e}'.format(result['integral'], result['error']))
